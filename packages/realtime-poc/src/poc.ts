...
async function main() {
  ...
  await db.query(
    "insert into public.poc06_validation_events(id, payload) values ($1::uuid, $2::jsonb)",
  ...
  const subscriberChannel = subscriber
    .channel("poc06:validation:authorized", { config: { private: true } })
  ...
  const publisherChannel = publisher.channel("poc06:validation:authorized", {
    config: { private: true },
  });
  ...
  const forbiddenChannel = unauthorized.channel("poc06:validation:forbidden", {
    config: { private: true },
  });
  ...
  const persisted = await db.query<{
    ...
    `
      ...
      from public.poc06_validation_events
      where id = any($1::uuid[])
